...
import { ChoroplethTooltipMatcherTypes, type TooltipParams } from './types';

export const LIGHT_GREY: string = '#CBD2DB';
export const DARK_GREY: string = '#515457';
...
export const mapKeyToColor = (
	...
	emptyValueColor: string = DEFAULT_COLORS.Default
): { [s: string]: string } => {
	...
	switch (colorScale.type) {
		case ColorScaleTypes.Palette: {
			...
			colorScale.colors.forEach((_color: string, i: number) => {
				if (i === 0) {
	...
	const dataMapping: { [s: ChoroplethDataValue['x']]: string } = {};
	values.forEach(({ x, y }) => {
...
export const getFixedTooltips = (
	...
	const popups = shapeKeys.map((shapeKey) => {
		const matchedFeature = features.find((feature) => feature.properties?.[matchKey] === shapeKey);
		if (matchedFeature) {
...
export const computeFilterExpression = (
	filterConfig: MapFilter
): maplibregl.ExpressionSpecification => {
	const { key, value } = filterConfig;
	const filterMatchExpression: maplibregl.ExpressionSpecification = [
		'in',
		['downcase', ['get', key]],
		['literal', []]
	];
...
export const computeMatchExpression = (
	...
	const matchExpression: ['match', maplibregl.ExpressionSpecification] = [
		'match',
		['get', matchKey]
	];
	const groupByColors: maplibregl.ExpressionInputType[] = [];
