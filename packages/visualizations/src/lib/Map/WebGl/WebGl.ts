...
export default class MapPOI {
	...
	private unhighlightFeature(feature: ActiveFeatureType) {
		...
		this.queue((map) => {
			...
			switch (layer.type) {
				...
				case 'circle':
					// eslint-disable-next-line no-case-declarations
					const circleRadius = (layer as CircleLayerSpecification).paint?.['circle-radius'];
					/*
	...
	private renderFeaturesNavigationControls() {
		...
		if (availableFeaturesTotal > 1) {
			const activeFeatureHumanIndex = this.availableFeaturesOnClick.indexOf(this.activeFeature) + 1;
