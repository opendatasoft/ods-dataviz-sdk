...
// Column types whose CSS truncates content (see CellContent.svelte styles)
...
function tooltipOnOverflow(el: HTMLElement, dataFormat?: string) {
    ...
    if (!dataFormat || TRUNCATING_FORMATS.includes(dataFormat)) create();

    return {
        update(newDataFormat?: string) {
            if ((!newDataFormat || TRUNCATING_FORMATS.includes(newDataFormat)) && !action) create();
            else if (newDataFormat && !TRUNCATING_FORMATS.includes(newDataFormat) && action)
                teardown();
        },
        destroy: teardown,
    };
