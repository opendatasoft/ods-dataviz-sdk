import { BaseFilter, HandleFilterCallback } from '@opendatasoft/visualizations';
...
export default function wrap<Options, ComponentClass extends BaseFilter<Options>>(
    ...
    return forwardRef((props: FilterProps<Options>, forwardedRef: ForwardedRef<HTMLElement>) => {
        const { tag, handleFilter, options, ...elementProps } = props;
        ...
        useEffect(() => {
            componentRef.current?.updateHandleFilter(handleFilter);
        }, [handleFilter]);
        ...
        useEffect(() => {
            ...
            if (container) {
                const component = new ComponentConstructor(container, handleFilter, options);
